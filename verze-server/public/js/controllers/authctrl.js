...
'LanguageFormat',
'BadFilter',
function($rootScope, $scope, $state, $ionicHistory, $ionicPopup, $ionicPopover, $ionicLoading, auth, users, Misc, Language, LanguageFormat, BadFilter){
...
//	$scope.updateLang = LanguageFormat.updateLang;
	...
	$(document).ready(function(){
		...
		$('.what-is-your-main-language-trans').text($scope.language.whatIsYourMainLanguage);
		$('.what-is-your-level-of-fluency-trans').text($scope.language.whatIsYourLevelOfFluency);
		$('.what-language-do-you-want-to-learn-trans').text($scope.language.whatLanguageDoYouWantToLearn);
...
//			return;
		...
		auth.register($scope.user).error(function(error){
			...
			auth.whoAmI(auth.currentUsername()).success(function(data){
				...
				users.addLanguage({
					name: $scope.mainLanguageSelected,
					level: $scope.levelSelected,
	...
	$scope.whichCourse = function() {
		...
		else*/ if($scope.languageSelected=="Chinese") $state.go('viewCourse', { course: "59b9d62db7bcaed219bec6cd" });
		else if($scope.languageSelected=="Spanish") $state.go('viewCourse', { course: "5a8bbcbdd6ed739b42f160fb" });
	}
	...
	$scope.mainLanguageSelected = "";
	$scope.languageSelected = "";
	$scope.levelSelected = "";
	
	$scope.getSlideActive = function(type, what) {
		if(type=='main-language') {
			$scope.mainLanguageSelected = what;
